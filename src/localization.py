...
class PatchLocalization:
    ...
    def plot_heatmap(self,image,label=1,alpha=0.25,color='r'):
        ...
        fig, ax = plot_patch_heatmap(self.inds,self.prediction,self.patch_size,
                              image,label=label,alpha=alpha,color=color)
        return fig,ax
        
    def pixel_mask(self,label,mask_dims=None,threshold=None):
        
        mask = pixel_mask(self.inds,self.prediction,self.patch_size,
                          label=label,mask_dims=mask_dims,threshold=threshold)
               
        return mask
    
    def score_f1(self,ground_truth_mask,upper_pct=75,lower_pct=25,
                 invert_mask=False):
        ...
        y = patch_score_from_mask(ground_truth_mask,self.inds,self.patch_size,
...
#        p[self.prediction==label] = 1
    ...
    def score_mcc(self,ground_truth_mask,upper_pct=75,lower_pct=25,
                 invert_mask=False):
...
class dbScorer:
    ...
    def score_best_thresh_per_image(self,score_type='mcc',
                                   thresh=np.linspace(0,1,11),quiet=False):
        if score_type == 'f1':
    ...
    def score_best_thresh_for_db_imavg(self,score_type='mcc',thresh=np.linspace(0,1,21),quiet=False):
        db_score_list = []
        for tt in tqdm(thresh,desc='Scoring DB',disable=quiet):
            score, _ = self.score_best_thresh_per_image(score_type,thresh=[tt],quiet=True)
    ...
    def score_best_thresh_pix_total(self,score_type,thresh=np.linspace(0,1,21),quiet=False):
        maxscore = 0
        bestthresh = None
...
def pixel_loc_from_patch_pred(prediction,inds,patch_size,image_shape,threshold,
                              smoothing='Gaussian',smoothing_window=32,
                              normalization=True):
#inds = sg.inds
...
#threshold = 0.33
    ...
    pixel_prediction = patchloc.pixel_mask(label=1,mask_dims=image_shape,threshold=None)
    
    
    #smoothing
    if smoothing == 'Gaussian':
        pixel_prediction = gaussian_filter(pixel_prediction,
    ...
    if normalization == True:
        ...
        if smoothing == 'Gaussian':
            ...
            coverage = gaussian_filter(coverage,(smoothing_window,smoothing_window),
                                       mode='reflect')
...
def pixel_pred_from_patch_pred(prediction,inds,patch_size,image_shape,
                              smoothing='Gaussian',smoothing_window=32,
                              normalization=True):
#inds = sg.inds
#patch_size=sg.patch_size
...
def plot_patch_heatmap(inds,prediction,patch_size,image,label=1,alpha=0.25,color='r'):
...
def compare_pixels_to_mask(inds,patch_prediction,patch_size,mask_dims,threshold,label=1,normalize=True):
    
    #convert patch scores to pixel map, each pixel is the sum of patches that detect it
    patch_prediction_map = pixel_mask(inds,patch_prediction,patch_size,label=label,mask_dims=mask_dims)
...
def patch_coverage(inds,patch_size,mask_dims,scale=None):
    coverage = pixel_mask(inds,np.zeros(len(inds)),patch_size,label=0,mask_dims=mask_dims,threshold=None)
