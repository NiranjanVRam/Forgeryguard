...
def read_and_decode_pairs_key(tfrecords_file, batch_size, name, n_classes, patch_size = 256,num_threads = 8):
    ...
    patch1_batch, patch2_batch, label_batch, key_batch = tf.train.batch([patch1,patch2,label,key],
    ...
    return patch1_batch, patch2_batch, tf.reshape(label_batch, [batch_size, n_classes]), key_batch

def read_and_decode_pairs(tfrecords_file, batch_size, name, n_classes, patch_size = 256,num_threads = 8):
    ...
    patch1_batch, patch2_batch, label_batch = tf.train.batch([patch1,patch2,label],
                                              batch_size=batch_size,
                                              num_threads=num_threads,
                                              capacity=2000,
                                              name=name)
...
def CompareNet_v1_96(x1,x2,phase,nprefilt=6,nb12=2048,nb3=64):
    ...
    MISL_output1 = MISLNet96(x1, phase, nprefilt=nprefilt,reuse=False) #feature extractor for patch 1
    MISL_output2 = MISLNet96(x2, phase, nprefilt=nprefilt,reuse=True) #feature extractor for patch 2, with same weights as 1
...
def CompareNet_v1_64(x1,x2,phase,nprefilt=6,nb12=2048,nb3=64):
    ...
    MISL_output1 = MISLNet64(x1, phase, nprefilt=nprefilt,reuse=False) #feature extractor for patch 1
    MISL_output2 = MISLNet64(x2, phase, nprefilt=nprefilt,reuse=True) #feature extractor for patch 2, with same weights as 1
